...
class TTSService:
    def __init__(self):
        ...
        if not self.is_available:
            logger.warning("ELEVENLABS_API_KEY not found or ElevenLabs not available. TTS will be disabled.")
            return
    ...
    def synthesize(self, text: str, voice: str = "Rachel") -> Optional[bytes]:
        ...
        try:
            logger.info(f"Generating TTS audio for text: '{text[:50]}...' with voice: {voice}")
            ...
            logger.info(f"TTS audio generated successfully - {len(audio_bytes)} bytes from {chunk_count} chunks")
            return audio_bytes
    ...
    async def generate_audio_url(self, text: str, voice_id: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            logger.info(f"Audio data generated, length: {len(audio_data) if audio_data else 0} bytes")
