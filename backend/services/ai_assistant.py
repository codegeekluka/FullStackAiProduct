...
from backend.database.db_models import Recipe, Instruction, Ingredient, UserSession, UserConversation
...
class AIAssistantService:
    ...
    def update_recipe_embeddings(self, db: Session, recipe_id: int):
        ...
        try:
            ...
            recipe.embedding = np.array(self.generate_embeddings(recipe_text))
            
            # Update ingredient embeddings
            ingredients = db.query(Ingredient).filter(Ingredient.recipe_id == recipe_id).all()
            for ingredient in ingredients:
                ingredient.embedding = np.array(self.generate_embeddings(f"Ingredient: {ingredient.ingredient}"))
            
            # Update instruction embeddings
            instructions = db.query(Instruction).filter(Instruction.recipe_id == recipe_id).all()
            for i, instruction in enumerate(instructions):
                # Add step number for better context
                instruction_text = f"Step {i+1}: {instruction.description}"
                instruction.embedding = np.array(self.generate_embeddings(instruction_text))
    ...
    def hybrid_retrieve(self, db: Session, recipe_id: int, query: str, top_k: int = 8) -> List[Dict]:
        """
        ...
        try:
            ...
            ingredient_results = db.query(Ingredient).filter(
                Ingredient.recipe_id == recipe_id,
                Ingredient.embedding.isnot(None)
            ).order_by(
                Ingredient.embedding.cosine_distance(query_embedding)
            ).limit(top_k // 2).all()
            
            # Retrieve relevant instructions
            instruction_results = db.query(Instruction).filter(
                Instruction.recipe_id == recipe_id,
                Instruction.embedding.isnot(None)
            ).order_by(
                Instruction.embedding.cosine_distance(query_embedding)
            ).limit(top_k // 2).all()
            ...
            for ingredient in ingredient_results:
                results.append({
                    ...
                    "id": ingredient.id,
                    "similarity_score": self._calculate_similarity(query_embedding, ingredient.embedding)
                })
            ...
            for instruction in instruction_results:
                results.append({
                    ...
                    "id": instruction.id,
                    "similarity_score": self._calculate_similarity(query_embedding, instruction.embedding)
                })
    ...
    def create_cooking_context(self, db: Session, recipe_id: int, retrieved_chunks: List[Dict]) -> str:
        ...
        try:
            ...
            for chunk in retrieved_chunks:
                if chunk["type"] == "ingredient":
                    ...
                    context_parts.append(f"Step {chunk['step_number']}: {chunk['content']}")
    ...
    def get_conversation_history(self, db: Session, session_id: str, limit: int = MAX_CONVERSATION_HISTORY) -> List[Dict]:
        """Get recent conversation history for context"""
        try:
            conversations = db.query(UserConversation).filter(
                UserConversation.session_id == session_id
            ).order_by(
                UserConversation.timestamp.desc()
            ).limit(limit).all()
    ...
    def generate_ai_response(self, query: str, context: str, conversation_history: List[Dict]) -> str:
...
Please provide a helpful response:"""
    ...
    def create_session(self, db: Session, user_id: int, recipe_id: int) -> str:
        ...
        try:
            ...
            logger.info(f"Created new session {session_id} for user {user_id} and recipe {recipe_id}")
            return session_id
    ...
    def log_conversation(self, db: Session, session_id: str, role: str, message: str, 
                        query_type: Optional[str] = None, response_time_ms: Optional[int] = None):
