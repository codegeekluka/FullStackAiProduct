...
async def start_cooking_session(
    ...
    try:
        logger.info(f"Starting cooking session for recipe {request.recipe_id} by user {current_user.id}")
            ...
            try:
                ...
                def generate_embeddings_async():
                    try:
                        ...
                        try:
                            ai_assistant.update_recipe_embeddings(background_db, request.recipe_id)
                            logger.info(f"Successfully generated embeddings for recipe {request.recipe_id}")
                        finally:
                            background_db.close()
                    except Exception as e:
                        logger.error(f"Failed to generate embeddings for recipe {request.recipe_id}: {e}")
                ...
                logger.error(f"Failed to start embeddings generation for recipe {request.recipe_id}: {e}")
                # Don't fail the session start if embeddings generation fails
        else:
            logger.info(f"Recipe {request.recipe_id} already has embeddings")
        
        # Create new session
        logger.info(f"Creating session for user {current_user.id} and recipe {request.recipe_id}")
        session_id = ai_assistant.create_session(db, current_user.id, request.recipe_id)
        logger.info(f"Created session {session_id}")
        
        # Get session info
        session = db.query(UserSession).filter(UserSession.session_id == session_id).first()
        
        logger.info(f"Successfully started cooking session {session_id} for {recipe.title}")
...
async def ask_ai_assistant(
    ...
    try:
        ...
        try:
            ...
            logger.warning("TTS service may not be properly configured (missing ELEVENLABS_API_KEY)")
        ...
        current_step = ai_assistant.extract_current_step(ai_response, session.current_step)
...
async def process_audio_input(
    ...
    try:
        logger.info(f"Processing audio upload for session {session_id} by user {current_user.id}")
        ...
        if not session:
            logger.error(f"Session {session_id} not found or inactive for user {current_user.id}")
            raise HTTPException(status_code=404, detail="Session not found or inactive")
...
async def generate_tts(
    ...
    try:
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": "attachment; filename=tts_response.mp3"
            }
        )
    except Exception as e:
