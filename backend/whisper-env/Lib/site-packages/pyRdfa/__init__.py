...
err_absolute_reference = (
    "CURIE Reference part contains an authority part: %s (in '%s'); ignored"
)
...
err_undefined_terms = (
    "'%s' is used as a term, but has not been defined as such; ignored"
)
...
err_prefix_redefinition = (
    "Prefix '%s' (defined in the initial RDFa context or in an ancestor) is redefined"
)
...
from .host import HostLanguage, MediaTypes, content_to_host_language, preferred_suffixes
from .options import Options
from .parse import parse_one_node
from .state import ExecutionContext
from .transform import empty_safe_curie, top_about, vocab_for_role
...
builtInTransformers = [empty_safe_curie, top_about, vocab_for_role]
    ...
    def graph_from_source(self, name, graph=None, rdfOutput=False, pgraph=None):
        ...
        try:
            ...
            try:
                ...
                if not rdfOutput:
                    raise Exception(ex.msg)
                err = self.options.add_error(
                    "HTTP Error: %s (%s)" % (h.http_code, h.msg), HTError, name
                )
            ...
            try:
                ...
                if self.options.host_language == HostLanguage.html5:
                    ...
                    try:
                        ...
                        self.rdfa_version = adjust_html_version(
                            stream, self.rdfa_version
                        )
                    except:
                    ...
                    dom = parse(stream)
                    adjusted_host_language, version = adjust_xhtml_and_version(
                        dom, self.options.host_language, self.rdfa_version
                    )
...
def processURI(uri, outputFormat, form={}):
    ...
    check_lite = (
        "rdfa_lite" in list(form.keys())
        and form.getfirst("rdfa_lite").lower() == "true"
    )
    ...
    if "extras" in list(form.keys()) and form.getfirst("extras").lower() == "true":
        ...
        if (
            "extra-meta" in list(form.keys())
            and form.getfirst("extra-meta").lower() == "true"
        ):
            transformers.append(meta_transform)
        if (
            "extra-openid" in list(form.keys())
            and form.getfirst("extra-openid").lower() == "true"
        ):
            transformers.append(OpenID_transform)
        if (
            "extra-dc" in list(form.keys())
            and form.getfirst("extra-dc").lower() == "true"
        ):
    ...
    processor = pyRdfa(
        options=options, base=base, media_type=media_type, rdfa_version=rdfa_version
    )
