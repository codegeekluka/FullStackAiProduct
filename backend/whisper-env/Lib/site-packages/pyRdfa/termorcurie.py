...
from . import (
    RDFA_VOCAB,
    IncorrectPrefixDefinition,
    PrefixRedefinitionWarning,
    UnresolvableReference,
    err_absolute_reference,
    err_bnode_local_prefix,
    err_col_local_prefix,
    err_fragment_reference,
    err_invalid_prefix,
    err_missing_URI_prefix,
    err_no_default_prefix,
    err_non_ncname_prefix,
    err_prefix_and_xmlns,
    err_prefix_redefinition,
    err_query_reference,
    err_redefining_URI_as_prefix,
    err_xmlns_deprecated,
)
...
class TermOrCurie:
    ...
    def __init__(self, state, graph, inherited_state):
        ...
        def check_prefix(pr):
            ...
            if pr in uri_schemes:
                ...
                state.options.add_warning(
                    err_redefining_URI_as_prefix % pr, node=state.node.nodeName
                )
        ...
        if state.rdfa_version >= "1.1":
            ...
            if (
                self.state.node.hasAttribute("vocab")
                and self.state.node.getAttribute("vocab") == ""
            ):
                self.default_term_uri = default_vocab.vocabulary
            else:
                ...
                if def_term_uri and def_term_uri != "":
                    ...
                    self.graph.add(
                        (URIRef(self.state.base), RDFA_VOCAB, URIRef(def_term_uri))
                    )
        else:
            self.default_term_uri = None
        ...
        for i in range(0, state.node.attributes.length):
            ...
            if attr.name.find("xmlns:") == 0:
                ...
                if prefix != "":  # exclude the top level xmlns setting...
                    if (
                        state.rdfa_version >= "1.1"
                        and state.options.host_language in warn_xmlns_usage
                    ):
                        state.options.add_warning(
                            err_xmlns_deprecated % prefix,
                            IncorrectPrefixDefinition,
                            node=state.node.nodeName,
                        )
                    if prefix == "_":
                        state.options.add_warning(
                            err_bnode_local_prefix,
        ...
        if state.rdfa_version >= "1.1" and state.node.hasAttribute("prefix"):
            ...
            if pr != None:
                ...
                for i in range(len(pr_list) - 2, -1, -2):
                    ...
                    if i == len(pr_list) - 1:
                        state.options.add_warning(
                            err_missing_URI_prefix % (prefix, pr),
                            node=state.node.nodeName,
                        )
                        break
                    else:
                    ...
                    if prefix[-1] != ":":
                        ...
                        if prefix == "":
                            ...
                            if ncname.match(prefix):
                                ...
                                if (
                                    prefix in xmlns_dict and xmlns_dict[prefix] != uri
                                ) or (
                                    real_prefix in xmlns_dict
                                    and xmlns_dict[real_prefix] != uri
                                ):
                                    state.options.add_warning(
                                        err_prefix_and_xmlns
                                ...
                                state.options.add_warning(
                                    err_non_ncname_prefix % (prefix, pr),
                                    IncorrectPrefixDefinition,
                                    node=state.node.nodeName,
                                )
        ...
        if len(ns_dict) == 0:
            ...
            self.ns = {}
            for key in inherited_prefixes:
                self.ns[key] = inherited_prefixes[key]
            for key in ns_dict:
                if (
                    key in inherited_prefixes
                    and ns_dict[key] != inherited_prefixes[key]
                ) or (
                    key in self.default_prefixes
                    and ns_dict[key] != self.default_prefixes[key][0]
                ):
                    state.options.add_warning(
                        err_prefix_redefinition % key,
                        PrefixRedefinitionWarning,
                        node=state.node.nodeName,
                    )
        ...
        if len(xmlns_dict) == 0 and inherited_state:
            ...
            if inherited_state:
                for key in inherited_state.term_or_curie.xmlns:
                    self.xmlns[key] = inherited_state.term_or_curie.xmlns[key]
                for key in xmlns_dict:
                    self.xmlns[key] = xmlns_dict[key]
            else:
    ...
    def _check_reference(self, val):
        ...
        if netloc != "" and self.state.rdfa_version >= "1.1":
            self.state.options.add_warning(
                err_absolute_reference % (netloc, val),
                UnresolvableReference,
                node=self.state.node.nodeName,
            )
            return False
        elif not char_check(query):
            self.state.options.add_warning(
                err_query_reference % (query, val),
                UnresolvableReference,
                node=self.state.node.nodeName,
            )
            return False
            ...
            self.state.options.add_warning(
                err_fragment_reference % (fragment, val),
                UnresolvableReference,
                node=self.state.node.nodeName,
            )
            return False
