...
from . import (
    UnresolvablePrefix,
    UnresolvableTerm,
    err_illegal_safe_CURIE,
    err_no_CURIE_in_safe_CURIE,
    err_non_legal_CURIE_ref,
    err_undefined_CURIE,
    err_undefined_terms,
    err_URI_scheme,
)
    ...
    def __init__(
        ...
        if inherited_state:
            ...
            if node.hasAttribute("version"):
                ...
                if (
                    top_version.find("RDFa 1.0") != -1
                    or top_version.find("RDFa1.0") != -1
                ):
                    self.rdfa_version = "1.0"
                elif (
                    top_version.find("RDFa 1.1") != -1
                    or top_version.find("RDFa1.1") != -1
                ):
    ...
    def _URI(self, val):
        ...
        def create_URIRef(uri, check=True):
            ...
            if check and urlsplit(val)[0] not in uri_schemes:
                self.options.add_warning(
                    err_URI_scheme % val.strip(), node=self.node.nodeName
                )
            return URIRef(val)
    ...
    def _CURIEorURI(self, val):
        ...
        if self.rdfa_version >= "1.1":
            ...
            if retval == None:
                ...
                if safe_curie:
                    self.options.add_warning(
                        err_no_CURIE_in_safe_CURIE % val,
                        UnresolvablePrefix,
                        node=self.node.nodeName,
                    )
                    return None
                ...
                if (
                    isinstance(retval, BNode) == False
                    and urlsplit(str(retval))[0] == ""
                ):
