...
def handle_embeddedRDF(node, graph, state):
    ...
    if state.options.embedded_rdf:
        ...
        if (
            state.options.host_language in accept_embedded_turtle
            and node.nodeName.lower() == "script"
        ):
            if node.hasAttribute("type") and node.getAttribute("type") == "text/turtle":
                ...
                try:
                    ...
                    state.options.add_info(
                        "The output graph includes triples coming from an embedded Turtle script"
                    )
                except:
                    _type, value, _traceback = sys.exc_info()
                    state.options.add_error(
                        "Embedded Turtle content could not be parsed (problems with %s?); ignored"
                        % value
                    )
            return True
        elif (
            state.options.host_language in accept_embedded_rdf_xml
            and node.localName == "RDF"
            and node.namespaceURI == "http://www.w3.org/1999/02/22-rdf-syntax-ns#"
        ):
            rdf = StringIO(return_XML(state, node))
            try:
                graph.parse(rdf)
                state.options.add_info(
                    "The output graph includes triples coming from an embedded RDF/XML subtree"
                )
            except:
                _type, value, _traceback = sys.exc_info()
                state.options.add_error(
                    "Embedded RDF/XML content could not parsed (problems with %s?); ignored"
                    % value
                )
            return True
        else:
