...
#######################################################################
def parse_one_node(
    node, graph, parent_object, incoming_state, parent_incomplete_triples
):
...
def _parse_1_1(node, graph, parent_object, incoming_state, parent_incomplete_triples):
    ...
    def lite_check():
        if state.options.check_lite and state.options.host_language in [
            HostLanguage.html5,
            HostLanguage.xhtml5,
            HostLanguage.xhtml,
        ]:
            if (
                node.tagName == "link"
                and node.hasAttribute("rel")
                and state.term_or_curie.CURIE_to_URI(node.getAttribute("rel")) != None
            ):
                state.options.add_warning(
                    "In RDFa Lite, attribute @rel in <link> is only used in non-RDFa way (consider using @property)",
                    node=node,
                )
    ...
    if (
        state.options.embedded_rdf
        and node.nodeType == node.ELEMENT_NODE
        and handle_embeddedRDF(node, graph, state)
    ):
        return
    ...
    if (
        state.options.host_language in host_dom_transforms
        and node.nodeType == node.ELEMENT_NODE
    ):
    ...
    if not has_one_of_attributes(
        ...
        for n in node.childNodes:
            if n.nodeType == node.ELEMENT_NODE:
                parse_one_node(
                    n, graph, parent_object, state, parent_incomplete_triples
                )
        return
    ...
    if node.hasAttribute("property"):
        ProcessProperty(
            node, graph, current_subject, state, typed_resource
        ).generate_1_1()
...
def _parse_1_0(node, graph, parent_object, incoming_state, parent_incomplete_triples):
    ...
    #
    if not has_one_of_attributes(
        node, "href", "resource", "about", "property", "rel", "rev", "typeof", "src"
    ):
        # nop, there is nothing to do here, just go down the tree and return...
        for n in node.childNodes:
            if n.nodeType == node.ELEMENT_NODE:
