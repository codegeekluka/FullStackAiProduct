...
def parse_http_datetime(datestring, utc_tzinfo=None, strict=False):
    ...
    m = re.match(
        r"(?P<DOW>[a-z]+), (?P<D>\d+) (?P<MON>[a-z]+) (?P<Y>\d+) (?P<H>\d+):(?P<M>\d+):(?P<S>\d+(\.\d+)?) (?P<TZ>[a-zA-Z0-9_+]+)$",
        datestring,
        re.IGNORECASE,
    )
    if not m and not strict:
        m = re.match(
            r"(?P<DOW>[a-z]+) (?P<MON>[a-z]+) (?P<D>\d+) (?P<H>\d+):(?P<M>\d+):(?P<S>\d+) (?P<Y>\d+)$",
            datestring,
            re.IGNORECASE,
        )
        if not m:
            m = re.match(
                r"(?P<DOW>[a-z]+), (?P<D>\d+)-(?P<MON>[a-z]+)-(?P<Y>\d+) (?P<H>\d+):(?P<M>\d+):(?P<S>\d+(\.\d+)?) (?P<TZ>\w+)$",
                datestring,
                re.IGNORECASE,
            )
    if not m:
...
class ParseError(ValueError):
    ...
    def __str__(self):
        if self.at_position >= len(self.input_string):
            ...
            return "%s\n\tOccured near %s" % (
                self.args[0],
                repr(self.input_string[self.at_position : self.at_position + 16]),
            )
...
def parse_token_or_quoted_string(s, start=0, allow_quoted=True, allow_token=True):
    ...
    while pos < len(s):
        ...
        if c == "\\" and has_quote:
            ...
            if pos == len(s):
                raise ParseError(
                    "End of string while expecting a character after '\\'", s, pos
                )
            s2 += s[pos]
            pos += 1
...
def _test_comments():
    ...
    def _testrm(a, b, collapse):
        ...
        if b != b2:
            ...
            print(
                "   remove_comments( %s, collapse_spaces=%s ) -> %s"
...
def parse_range_spec(s, start=0):
    ...
    if first is None and last is None:
        raise ParseError(
            "Byte range can not omit both first and last indices.", s, start
        )
...
def parse_range_header(header_value, valid_units=("bytes", "none")):
    ...
    if k < len(header_value):
        raise ParseError(
            "Range header has unexpected or unparsable characters", header_value, k
        )
    return ranges
                ...
                for p, v in parmlist:
                    if p == "q" and qvalue is None:
                        try:
                            ...
                            raise ParseError(
                                "qvalue must be a floating point number", s, pos
                            )
                        if qvalue < 0 or qvalue > 1:
                            raise ParseError(
                                "qvalue must be between 0 and 1, inclusive", s, pos
                            )
                    elif qvalue is None:
...
class content_type(object):
    ...
    def __str__(self):
        ...
        if self.parmdict:
            extra = "; ".join(
                [
                    "%s=%s" % (a[0], quote_string(a[1], False))
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and self.parmdict == other.parmdict
        )
    ...
    media_type = property(
        _get_media_type,
        doc="Returns the just the media type 'type/subtype' without any paramters (read-only).",
    )
...
def acceptable_charset(
    accept_charset_header, charsets, ignore_wildcard=True, default="ISO-8859-1"
):
    """
...
def parse_accept_language_header(header_value):
    ...
    for token, langparms, q, acptparms in alist:
        if langparms:
            raise ParseError(
                "Language tag may not have any parameters", header_value, 0
            )
...
def acceptable_language(
    accept_header, server_languages, ignore_wildcard=True, assume_superiors=True
):
