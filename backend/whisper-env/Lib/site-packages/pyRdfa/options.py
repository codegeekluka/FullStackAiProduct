...
from . import RDFA_Error, RDFA_Info, RDFA_Warning, ns_distill, ns_rdfa, ns_xsd
from .host import (
    HostLanguage,
    content_to_host_language,
    predefined_1_0_rel,
    require_embedded_rdf,
)
    ...
    def add_triples(self, msg, top_class, info_class, context, node):
        ...
        self.graph.add(
            (
                ...
                ns_dc["date"],
                Literal(
                    datetime.datetime.utcnow().isoformat(), datatype=ns_xsd["dateTime"]
                ),
            )
        )
...
class Options:
    ...
    def __str__(self):
        ...
        return retval % (
            self.space_preserve,
            self.output_processor_graph,
            self.output_default_graph,
            self.host_language,
            self.embedded_rdf,
            self.rdfa_sem,
            self.vocab_cache,
    ...
    def add_warning(
        self, txt, warning_type=None, context=None, node=None, buggy_value=None
    ):
        ...
        """
        if (
            warning_type == ns_rdfa["UnresolvedTerm"]
            and buggy_value in predefined_1_0_rel
        ):
            return
        return self.processor_graph.add_triples(
            txt, RDFA_Warning, warning_type, context, node
        )
    ...
    def add_info(self, txt, info_type=None, context=None, node=None, buggy_value=None):
        ...
        if self.add_informational_messages:
            return self.processor_graph.add_triples(
                txt, RDFA_Info, info_type, context, node
            )
        else:
            return
    ...
    def add_error(self, txt, err_type=None, context=None, node=None, buggy_value=None):
        ...
        """
        return self.processor_graph.add_triples(
            txt, RDFA_Error, err_type, context, node
        )
