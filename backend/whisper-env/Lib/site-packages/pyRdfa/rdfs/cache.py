...
class CachedVocabIndex:
    ...
    def __init__(self, options=None):
        ...
        if not os.path.isdir(self.app_data_dir):
            try:
                ...
                if self.report:
                    options.add_info(
                        "Could not create the vocab cache area %s" % value,
                        VocabCachingInfo,
                    )
                return
        else:
            ...
            if not os.access(self.app_data_dir, os.R_OK):
                if self.report:
                    options.add_info(
                        "Vocab cache directory is not readable", VocabCachingInfo
                    )
                return
            if not os.access(self.app_data_dir, os.W_OK):
                if self.report:
                    options.add_info(
                        "Vocab cache directory is not writeable, but readable",
                        VocabCachingInfo,
                    )
                return
        ...
        if os.path.exists(self.index_fname):
            ...
            if os.access(self.app_data_dir, os.W_OK):
                ...
                try:
                    ...
                    if self.report:
                        options.add_info(
                            "Could not create the vocabulary index %s" % value,
                            VocabCachingInfo,
                        )
            else:
                if self.report:
                    options.add_info(
                        "Vocabulary cache directory is not writeable", VocabCachingInfo
                    )
    ...
    def add_ref(self, uri, vocab_reference):
        ...
        try:
            ...
            _t, value, _traceback = sys.exc_info()
            if self.report:
                self.options.add_info(
                    "Could not store the cache index %s" % value, VocabCachingInfo
                )
    ...
    def _give_preference_path(self):
        ...
        if CACHE_DIR_VAR in os.environ:
            ...
            if system == "win":
                ...
                return os.path.join(
                    os.path.expanduser("~"), self.preference_path[system]
                )
...
class CachedVocab(CachedVocabIndex):
    ...
    def __init__(self, URI, options=None, verify=True):
        ...
        if vocab_reference == None:
            ...
            if self.report:
                options.add_info(
                    "No cache exists for %s, generating one" % URI, VocabCachingInfo
                )
            ...
            if (
                options.refresh_vocab_cache == False
                and datetime.datetime.utcnow() <= self.expiration_date
            ):
                # We are fine, we can just extract the data from the cache and we're done
                if self.report:
                    options.add_info(
                        "Cache for %s is still valid; extracting the data" % URI,
                        VocabCachingInfo,
                    )
                ...
                try:
                    ...
                    if self.report:
                        options.add_info(
                            "Could not access the vocab cache %s (%s)" % (value, fname),
                            VocabCachingInfo,
                            URI,
                        )
            else:
                if self.report:
                    if options.refresh_vocab_cache == True:
                        options.add_info(
                            "Time check is bypassed; refreshing the cache for %s" % URI,
                            VocabCachingInfo,
                        )
                    else:
                        options.add_info(
    ...
    def _get_vocab_data(self, verify, newCache=True):
        ...
        self.graph, self.expiration_date = return_graph(
            self.uri, self.options, newCache, verify
        )
    ...
    def _store_caches(self):
        ...
        try:
            ...
            if self.report:
                self.options.add_info(
                    "Could not write cache file %s (%s)",
                    (fname, value),
                    VocabCachingInfo,
                    self.uri,
                )
        # Update the index
        self.add_ref(
            self.uri, (self.filename, self.creation_date, self.expiration_date)
        )
