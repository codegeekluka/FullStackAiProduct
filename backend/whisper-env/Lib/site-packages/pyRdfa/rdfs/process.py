...
from .. import HTTPError, RDFaError, VocabReferenceError
from ..host import MediaTypes
from ..utils import URIOpener
from . import (
    err_outdated_cache,
    err_unparsable_ntriples_vocab,
    err_unparsable_rdfa_vocab,
    err_unparsable_Turtle_vocab,
    err_unreachable_vocab,
...
def return_graph(uri, options, newCache=False, verify=True):
    ...
    def return_to_cache(msg):
        if newCache:
            options.add_warning(
                err_unreachable_vocab % uri, warning_type=VocabReferenceError
            )
        else:
            options.add_warning(
                err_outdated_cache % uri, warning_type=VocabReferenceError
            )
