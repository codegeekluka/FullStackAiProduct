...
option_parser.add_option(
    "--summary", default=False, action="store_true", help="show summary of differences"
)
option_parser.add_option(
    "--ignore-empty",
    default=False,
...
option_parser.add_option(
    "--json", default=False, action="store_true", help="Output results in JSON form"
)
...
def print_diffs(diffs, pretty=True, headers=True):
    ...
    for archive, points, total in diffs:
        if pretty:
            sys.stdout.write(
                "Archive %d (%d of %d datapoints differ)\n"
        ...
        json.dumps(
            {
                ...
                "path_b": path_b,
                "archives": [
                    {"archive": archive, "total": total, "points": points.__len__()}
                    for archive, points, total in diffs
                ],
            },
            sort_keys=True,
                ...
                "archives": [
                    {
                        ...
                        "datapoint": [
                            {"timestamp": p[0], "value_a": p[1], "value_b": p[2]}
                            for p in points
                        ],
                    }
                    for archive, points, total in diffs
...
def main():
    archive_diffs = whisper.diff(
        path_a, path_b, ignore_empty=options.ignore_empty, until_time=until_time
    )
    if options.summary:
        if options.json:
