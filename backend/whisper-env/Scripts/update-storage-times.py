...
DEFAULT_SCHEMA = {"match": re.compile(".*"), "retentions": "1m:7d"}
DEBUG = False
DRY_RUN = False
ROOT_PATH = ""
    ...
    for schema in schema_conf.items():
        ...
        if item == ".*":
            ...
            if item[0] == "^":
                item = item[1:]
            SCHEMA_LIST[item] = {
                "retentions": schema[1]["retentions"],
...
def _compare_retention(retention, tmp_path):
    ...
    cur_retention = [
        int(line.split(":")[1]) for line in cur_ret_list if "retention" in line
    ]
...
def _find_metrics(path):
    for f in scandir(path):
        if f.is_dir(follow_symlinks=False):
            ...
            if not f.is_file(follow_symlinks=False) or not f.name.endswith(".wsp"):
                continue
            yield f.path
...
def fix_metric(metric):
    ...
    if DRY_RUN:
        ...
        if DEBUG:
            ...
            res = subprocess.check_call(command_string, stdout=devnull)
...
def cli_opts():
    parser = argparse.ArgumentParser(
        "Correct storage settings on multiple whisper files"
    )
    parser.add_argument(
        "--cfg",
        action="store",
