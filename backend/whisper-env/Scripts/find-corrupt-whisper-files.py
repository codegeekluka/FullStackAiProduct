...
def walk_dir(base_dir, delete_corrupt=False, verbose=False):
    for dirpath, dirnames, filenames in os.walk(base_dir):
        ...
        whisper_files = (
            os.path.join(dirpath, i) for i in filenames if i.endswith(".wsp")
        )
        ...
        for f in whisper_files:
            ...
            if verbose:
                print(
                    "%s: %d points"
                    % (f, sum(i["points"] for i in info.get("archives", {})))
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--verbose", default=False, action="store_true", help="Display progress info"
    )
    parser.add_argument(
        "directories",
        type=str,
